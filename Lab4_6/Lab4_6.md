...
### Как запустить?
Для того чтобы запустить всю эту шайтан-машину, нам понадобится:
`npm i` - для установки всех зависимостей из файла `package.json`
Круто, зависимости на месте. Теперь нужно разобраться с нашей БД и ORM. Для этого делаем следующее:
...
`npx prisma migrate dev --name "migrationName"` - создадим миграцию нашей БД
`npx prisma migrate deploy` - сделаем деплой нашей созданной миграции 
`npx prisma generate` - сгенерируем наш prisma-client, который будет использоваться при работе ORM.
