...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	try
	{
		s = InitCA((char*)"25.19.247.43", (char*)"U:\\file.txt", _wtoi(argv[1]));
		while (true)
		{
			EnterCA(s);
			int k = 0;
			bool close = false;
			std::string line;
			std::ofstream ss("U:\\ris\\file.txt", std::ios::app);
			std::ifstream s2("U:\\ris\\file.txt", std::ios::app);
			...
			std::cin >> k;
				if (k == 1 ) {
					for (int i = 0; i < 1; i++)
					{
						...
						if (ss.is_open())
						{
							ss << "Client: " << s.clientnumber << " 123 " << asctime(timeinfo) << std::endl;
						}
						Sleep(500);
						LeaveCA(s);
					}
